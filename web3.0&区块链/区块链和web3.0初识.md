...
**定义**：运行在区块链上的**自动化程序**，能够在满足预设条件时自主执行协议条款，无需依赖第三方中介。
...
1. ​**确定性执行**
    ...
    - ​**实现保障**：区块链虚拟机隔离执行环境，禁用随机数（除非预言机输入）和系统时间依赖。
2. ​**触发与响应机制**
    - ​**事件驱动**：外部交易调用合约函数（如用户发起转账）。
    - ​**定时触发**：依赖区块链时间戳执行定时任务（如DAO定期分红）。
3. ​**密码学验证**
    - 用户通过私钥签名发起交易，合约验证签名合法性后执行操作。
4. ​**状态持久化**
